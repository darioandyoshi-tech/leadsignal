...
class FallbackAgentMail:
    def __init__(self, *args, **kwargs):
        raise RuntimeError("AgentMail not available")
...
class FallbackNIMClient:
    ...
    def analyze_market_event_deep(self, *args, **kwargs):
        return {
            "status": "error",
...
try:
    ...
    print(f"Warning: agentmail package not found: {e}")
    AGENTMAIL_AVAILABLE = False
    AgentMail = FallbackAgentMail

try:
    from nim_client import NVIDIANIMClient
    NIM_CLIENT_AVAILABLE = True
except ImportError as e:
    print(f"Warning: NVIDIA NIM client not available: {e}")
    NIM_CLIENT_AVAILABLE = False
    NVIDIANIMClient = FallbackNIMClient

NIM_AVAILABLE = AGENTMAIL_AVAILABLE and NIM_CLIENT_AVAILABLE
...
async def send_phase4_alert(use_nim_enhancement=True):
    ...
    try:
        ...
        print(f"Error initializing AgentMail client: {e}")
        return False
...
def main():
    ...
    print("=" * 50)
    print(f"AgentMail Available: {AGENTMAIL_AVAILABLE}")
    print(f"NIM Client Available: {NIM_CLIENT_AVAILABLE}")
    print(f"Full Hybrid Available: {NIM_AVAILABLE}")
    
    # Check if we have the minimum required components
    if not AGENTMAIL_AVAILABLE:
        print("[!] Error: AgentMail is required for alert sending")
        return 1
    
    # Run the alert sending
    result = asyncio.run(send_phase4_alert(use_nim_enhancement=NIM_CLIENT_AVAILABLE))
