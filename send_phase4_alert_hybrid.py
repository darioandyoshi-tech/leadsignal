...
Deep Analyzer: NVIDIA NIM (Deep-path: {nim_analysis.get('processing_time_ms', 0):.0f}ms if available)
    ...
    if use_nim_enhancement and NIM_CLIENT_AVAILABLE:
        try:
            ...
            if nim_client.is_ready():
                ...
                if nim_analysis.get("status") == "success":
                    print(f"[+] NIM deep analysis completed in {nim_analysis['processing_time_ms']:.0f}ms")
                else:
                    print(f"[-] NIM deep analysis failed: {nim_analysis.get('error', 'Unknown error')}")
            else:
                print("[*] NIM client not available, using fast-path only")
                
        except Exception as e:
            print(f"[-] Error initializing NIM client: {e}")
            nim_analysis = {"status": "error", "error": str(e)}
